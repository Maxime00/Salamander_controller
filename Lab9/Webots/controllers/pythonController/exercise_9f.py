...
def exercise_9f(world, timestep, reset):
    ...
    phase_offset = 3.14 #put optimal offset
    amp = 0.2
    ...
    parameter_set = [
        SimulationParameters(
            ...
            phase_lag=[0.6283185307179586, 0.6283185307179586, 0.6283185307179586, 0.6283185307179586, 
                        0.6283185307179586, 0.6283185307179586, 0.6283185307179586, 0.6283185307179586, 
                        0.6283185307179586, 0.6283185307179586, phase_offset, phase_offset, phase_offset, phase_offset],
            turn=None,
    ...
    for simulation_i, parameters in enumerate(parameter_set):
        ...
        run_simulation(
            ...
            logs="./logs/9famp/simulation_{}.npz".format(simulation_i)
        )
    """
...
def main():
    ...
    parameter_set = [
        ...
        for phase_offset in (np.linspace(-2*np.pi,2*np.pi,30)-np.pi)
...
def plot_9f(parameter_set):
    ...
    for i in range(len(parameter_set)):
        with np.load(path+'/logs/9famp/simulation_'+str(i)+'.npz',allow_pickle=True) as data:
            
            #? init for the computation
    ...
    plt.plot(amplitudes, results_vel)
