...
def plot_9c(parameter_set):
    ...
    perc_vel = np.zeros([len(parameter_set)])
    perc_en = np.zeros([len(parameter_set)])
    ...
    for i in range(len(parameter_set)):
        with np.load(path+'/logs/9c/simulation_'+str(i)+'.npz',allow_pickle=True) as data:
            ...
            results_en[i][2] = np.mean(np.sum(energy,1))
    ...
    plt.figure("Energy")
    plot_2d(results_en,['Amplitude Head [rad]', 'Amplitude Tail [rad]', 'Energy[J]'])
    plt.figure("Ratio")
